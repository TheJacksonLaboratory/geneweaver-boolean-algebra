"""Tests for the GeneValue and GeneValueFullHash schema classes."""
import pytest
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (GeneValue(symbol="A", value=0), GeneValueFullHash(symbol="B", value=1), False),
        (GeneValueFullHash(symbol="B", value=1), GeneValue(symbol="A", value=0), False),
        (1, GeneValueFullHash(symbol="A", value=1), False),
        ("A", GeneValueFullHash(symbol="A", value=1), False),
        (1, GeneValue(symbol="A", value=1), False),
        ("A", GeneValue(symbol="A", value=1), False),
    ],
)
def test_gene_values_equality(gene_value_0, gene_value_1, should_equal):
    """Test that we can compare GeneValue schemas."""
    if should_equal:
...
def test_can_hash_gene_values(gene_value):
    """Test that we can hash the GeneValue schemas."""
